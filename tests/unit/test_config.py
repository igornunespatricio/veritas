...
class TestSettings:
    ...
    def test_default_values(self):
        ...
        # Reload settings to use mocked env
        with patch("src.config.Settings.model_config", {"env_file": ""}):
            settings = Settings()
            assert settings.environment == "development"
            # API keys may be set from env, so just check type
            ...
            assert settings.tavily_api_key is None or isinstance(
                settings.tavily_api_key, str
            )
    ...
    def test_circuit_breaker_defaults(self):
        ...
        assert settings.circuit_cooldown_seconds == 30.0
        assert settings.circuit_timeout_seconds == 60.0
    ...
    def test_retry_defaults(self):
        ...
        assert settings.retry_max_backoff == 60.0

    def test_langsmith_defaults(self):
        """Test default LangSmith settings."""
        settings = Settings()
    ...
    def test_env_file_loading(self, tmp_path):
        ...
        with patch.dict(os.environ, {"PWD": str(tmp_path)}):
            with patch("src.config.settings") as mock_settings:
                ...
                # Note: In practice, Pydantic BaseSettings loads from .env automatically

    def test_extra_ignore(self):
        """Test that extra fields are ignored."""
        settings = Settings()
        # Should not raise even with extra env vars
...
class TestIsRetryableError:
    ...
    def test_api_error_is_retryable(self):
        """Test that APIError is retryable."""
        error = Exception("api_error: Something went wrong with the API")
        assert is_retryable_error(error) is True
